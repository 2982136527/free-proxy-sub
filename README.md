...
| **本地精选** (30) | `http://192.168.6.162:8080/clash-selected.yaml` | — (局域网直连，无需镜像) |
...
| **本地精选** (30) | `http://192.168.6.162:8080/sub-selected.b64` | — (局域网直连，无需镜像) |
...
> **镜像说明**：镜像使用的是 `gh-proxy.com`。如果失效可尝试 `gh.api.99988866.xyz`、`github.moeyy.xyz` 等同类服务，或将链接中的 `raw.githubusercontent.com` 替换为镜像地址。
> **本地精选**：由本地网络 G2532 板载服务每 30 分钟验证生成，筛选对国内网络实际可用的节点。仅限局域网内使用。
...
| `local-validate` (G2532 板载) | **每 ~7 分钟** | 板端 Debian 运行验证 → 生成局域网可用精选节点 → HTTP 直连 |
